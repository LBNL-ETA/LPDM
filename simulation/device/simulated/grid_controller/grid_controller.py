...
class GridController(Device):
    ...
    def power_source_update(self):
        ...
        if initial_success:
            ...
            if len(p_buyers) > 0 and p_seller and p_seller.load <= 1e-7:
                ...
                p_buyer = p_buyers[0]
                p_buy_orig = p_buyer.load

                p_diff = self.power_source_manager.total_capacity() - self.power_source_manager.total_load()
                self._logger.debug(self.build_message(
                    message="p_available_buy", tag="p_avail_buy", value=p_diff
                ))
                if p_diff > 1e-7:
                    # there is power available to purchase
                    p_buyer.load = p_diff
                if p_buyer.load != p_buy_orig:
                    self._logger.debug(self.build_message(
                        message="buy_power",
                        tag="buy_power",
                        value=p_buyer.load
                    ))
                    self.broadcast_buy_power(p_buyer.device_id, p_buyer.load)
    ...
    def log_utility_meters(self):
        ...
        if len(p_buyers):
            p_buyer = p_buyers[0]
            self._logger.debug(self.build_message(
                message="load for device {}".format(p_buyer),
                tag="buy_pwr",
                value=p_buyer.load
            ))

        p_sell = self.power_source_manager.get_utility_meter()
        if p_sell:
            self._logger.debug(self.build_message(
                message="load for device {}".format(p_sell),
                tag="sell_pwr",
                value=p_sell.load
            ))
