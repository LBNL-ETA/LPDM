...
class Battery(Device):
    ...
    def updateStateOfCharge(self, time):
        if self._is_charging:
            self._current_soc = ((self._capacity * self._current_soc) + (self.chargeRate() * (time - self._last_update_time))) / self._capacity
            self.tugLogAction(action="state_of_charge", is_initial_event=False, value=self._current_soc, description="")
        elif self._is_discharging:
            self._current_soc = ((self._capacity * self._current_soc) - (self._capacity * (time - self._last_update_time) / 3600.0)) / self._capacity
            self.tugLogAction(action="state_of_charge", is_initial_event=False, value=self._current_soc, description="")
