...
class Eud(Device):
        ...
        if type(self._daily_schedule) is list:
            ...
            for item in self._daily_schedule:
                if item['time_of_day_seconds'] > current_time_of_day_seconds:
                    new_ttie = int(self._time / (24 * 60 * 60)) + item['time_of_day_seconds']
                    next_event = item
                    break
    ...
    def calculateNewPowerLevel(self):
        ...
        if self._in_operation and self._price <= self._price_dim:
            return self._max_power_use
        elif self._in_operation and self._price <= self._price_off:
